...
def leer_sample(coleccion: str, geoserver: str, nombre: str) -> list[list]:
    ...
    return [[str(key), compactar_valor_sample(value)] for key, value in sample.items()]
...
SAMPLE_VALUE_LIMIT = 200


def compactar_valor_sample(value):
    if isinstance(value, str) and len(value) > SAMPLE_VALUE_LIMIT:
        return f"{value[:SAMPLE_VALUE_LIMIT - 1]}…"
    if isinstance(value, list):
        return [compactar_valor_sample(entry) for entry in value]
    if isinstance(value, dict):
        return {key: compactar_valor_sample(entry) for key, entry in value.items()}
    return value
