...
export function mostrarCapa(capa, directorio) {
    ...
    function mostrarNombre(capa) {
      return html`<div capa="nombre">
        ...
        <div class="nombreTexto">${capa.nombre}</div>
      </div>`
    }
    ...
    function mostrarGeoserver(geoserver) {
      return html`<div class="geoserver">${directorio[geoserver].descripcion}</div>`
    }
    ...
    function mostrarFechas(capa) {
      ...
      function mostrarFecha(nombre, fecha) {
        if (fecha) {
          return html`<div class="fecha">${nombre} ${fecha}</div>`
        } else {
          return ``
        }
      }
    ...
    function link(url, texto, clase="") {
      return html`<a class="enlace ${clase}" target="_blank" href="${url}" style="">${texto}</a>`
    }
  
    function wmsUrl(capa, formato, width=500, height=500) {
      const ows = directorio[capa.geoserver].ows
      if (ows.match(/\/ows$/g)) {
        return `${ows.replace("/\/ows$/i", "")}/wms/reflect?layers=${capa.nombre}&format=${formato}`
      } else {
    ...
    function linkWfs(capa, formato, texto, clase="enlaceWfs") {
      return link(
        `${directorio[capa.geoserver].ows}?service=wfs&request=GetFeature&typeName=${capa.nombre}&outputFormat=${formato}`,
        texto,
        clase
      )
    }
    ...
    function imagen(capa, width=50, height=50) {
      ...
      let url
      if (ows.match(/\/ows$/g)) {
        url = `${ows.replace("/\/ows$/i", "")}/wms/reflect?layers=${capa.nombre}&format=${formato}&width=${width}&height=${height}`
      } else {
      ...
      return html`<div class="imagen" style="background-image:url(${url});"></div>`
    }
    ...
    <div>
      <div class="nombreContenedor">
        ${mostrarNombre(capa)}
        ${mostrarDescripcion(capa.descripcion)}
